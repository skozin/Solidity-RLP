...
contract("RLPReader", async (accounts) => {
    ...
    it("correctly computes keccak256 hash of the data", async () => {
        const data = '0xdeadbeef';
        const rlpBytes = rlp.encode(Buffer.from(data.slice(2), 'hex'));
        const result = await helper.dataKeccak256.call(rlpBytes);
        assert.equal(result, web3.utils.keccak256(data), "bytes4");
    });

    it("correctly computes keccak256 hash of the data (nested list)", async () => {
    ...
    it("dataKeccak256 for empty data is the same as keccak256(new bytes(0))", async () => {
        ...
        const emptyListDataHash = await helper.dataKeccak256.call(emptyListRlpBytes);
        assert.equal(emptyListDataHash, EMPTY_BYTES_HASH, "empty list");

        const emptyBytesRlpBytes = rlp.encode(Buffer.alloc(0)); // 0x80
        const emptyBytesDataHash = await helper.dataKeccak256.call(emptyBytesRlpBytes);
